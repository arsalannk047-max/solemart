...
export default function SignupPage() {
  ...
  return (
    <div className="max-w-md mx-auto px-6 py-16">
      <h1 className="font-display text-3xl uppercase text-center mb-8">Create account</h1>
      <div className="bg-surface border border-line rounded-2xl p-7">
        ...
        {state?.message && <div className="bg-lime-100 text-green-800 text-sm font-semibold rounded-lg px-4 py-3 mb-4">{state.message}</div>}
        <form action={formAction}>
          ...
          <input name="password" type="password" minLength={6} required className="w-full border border-line rounded-lg px-3.5 py-2.5 mb-4 text-sm" />
          <SubmitButton />
        </form>
        <p className="text-center text-sm mt-4">
          Already have an account? <Link href="/login" className="text-azure font-semibold underline">Log in</Link>
        </p>
      </div>
    </div>
  );
